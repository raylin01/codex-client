...
function collectTextFromUserInputs(content: any[]): string[] {
  const texts: string[] = [];
...
function commandResultText(item: any): string {
  const lines: string[] = [];
...
function fileChangeResultText(item: any): string {
  const changes = Array.isArray(item?.changes) ? item.changes : [];
  const lines = [`Files changed: ${changes.length}`];
  if (typeof item?.status === 'string') lines.unshift(`Status: ${item.status}`);
...
function mcpResultText(item: any): string {
  if (item?.error) return `Error: ${safeJson(item.error, 800)}`;
  if (item?.result != null) return safeJson(item.result, 1200);
  return 'No MCP result available.';
}

function extractFirstPrompt(thread: Thread): string {
...
export function normalizeCodexThreadMessages(thread: Thread): SessionTranscriptMessage[] {
  ...
  turns.forEach((turn: any, turnIndex: number) => {
    ...
    items.forEach((wrappedItem: any, itemIndex: number) => {
      ...
      switch (itemType) {
        ...
        case 'agentMessage': {
          if (typeof item?.text === 'string' && item.text.trim()) blocks.push({ type: 'text', text: item.text.trim() });
          break;
        }
        case 'reasoning': {
        ...
        case 'plan': {
          if (typeof item?.text === 'string' && item.text.trim()) blocks.push({ type: 'plan', text: item.text.trim() });
          break;
        }
        ...
        default: {
          ...
          if (fallback) blocks.push({ type: 'text', text: `[${itemType}] ${fallback}` });
          break;
        }
      }
...
export async function listCodexSessionSummaries(
  client: Pick<CodexClient, 'listThreads'>,
  params: ThreadListParams = {}
): Promise<SessionBrowserSummary<Thread>[]> {
...
export async function readCodexSessionRecord(
  client: Pick<CodexClient, 'readThread'>,
  threadId: string,
  params: Omit<ThreadReadParams, 'threadId' | 'includeTurns'> = {}
): Promise<SessionBrowserRecord<Thread, unknown> | null> {
