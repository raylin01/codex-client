...
export class StructuredCodexClient extends EventEmitter {
  ...
  private turnFromRemote(turnId?: string, createIfMissing = false): CodexTurnHandle | null {
    ...
    if (!this.activeTurn && createIfMissing && this.threadId && turnId) {
      const handle = new CodexTurnHandle(
        `attached-${++this.turnCounter}`,
        this.threadId,
        '',
        { resumed: true, synthetic: true }
      );
